...
function hikeTrailTemplate(trail, placename) {
	return `
		<div class="trail-data block">
			<img src=${trail.imgSmall} alt="trail image" onerror="this.src='error-image.jpg'; this.alt='trail image coming soon';">
			<h3><a href=${trail.url} target="_blank">${trail.name}</a></h3>
			<p>${trail.location}</p>
			<p>Distance: ${trail.length} mi</p>
            <p>Rating: ${trail.stars}&#9733;</p>
            <p><a href="https://www.google.com/maps/search/?api=1&query=${placename}" target="_blank">Get Directions</a></p>
		</div>
	`;
}
